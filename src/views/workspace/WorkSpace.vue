<template>
  <div class="workspace">
    ...
    <el-tabs type="border-card">
      ...
      <el-tab-pane label="待我处理">
        <div class="no-data">暂无数据 😀</div>
      </el-tab-pane>
      <el-tab-pane :label="`我发起的(${myProcess.total})`">
        <el-table :header-cell-style="{background:'#f5f6f6'}" :data="myProcess.data" border style="width: 100%">
          <el-table-column prop="processDefinitionName" show-overflow-tooltip label="流程名称">
          </el-table-column>
          <el-table-column prop="startUser" show-overflow-tooltip label="发起人">
          </el-table-column>
          <el-table-column prop="startTime" show-overflow-tooltip label="开始时间">
          </el-table-column>
          <el-table-column prop="endTime" show-overflow-tooltip label="结束时间">
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleCurrentChange(pageNo) {
      this.getApplyList(pageNo)
    },
    async getApplyList(pageNo) {
      this.myProcess.pageNo = pageNo || 1
      ...
      if (res.data) {
        this.myProcess.data = res.data.result.records.map(item => {
          if (item.startTime) {
        ...
        this.myProcess.total = res.data.result.total
    ...
    submitForm(){
      this.$refs.processForm.validate(async valid => {
        if (valid) {
          const res = await getProcessDetail(this.selectForm.templateId)
          console.log(res)
          if (res.data.result) {
            const user = JSON.parse(sessionStorage.getItem('user'))
            processStart({
              formData: this.$refs['processForm'].formConfig.data,
              processDefinitionId: res.data.result.processDefinitionId,
              startUserInfo: user
            })
          }
