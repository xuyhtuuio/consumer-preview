    ...
    <g-table-card :title="title">
      ...
      <el-form :hide-required-asterisk="true" label-width="100px" class="ruleForm my-form ">
        ...
        <el-form-item :ref="`${item.id}Ref`" class="form-item" v-if="item.name==='TimePicker' && item.props.required" :key="index"  :prop="item.id" >
               <label slot="label">{{item.title}}<span :style="{'color':'red',opacity: !item.props.required && 0}"> * </span></label>
              <el-date-picker type="date" placeholder="选择日期" :format="item.props.format" v-model="item.value" :picker-options="startTime(item.gl)" style="width: 100%;"></el-date-picker>
               <div class="warn" v-if="item.isWarning"><warn-info :info="judgementWarnCpt(item)"></warn-info></div>
            </el-form-item>
            <el-form-item :ref="`${item.id}Ref`" class="form-item" v-if="item.name==='TimePicker' && !item.props.required" :key="index" :prop="item.id">
               <label slot="label">{{item.title}}<span :style="{'color':'red',opacity: !item.props.required && 0}"> * </span></label>
              <el-date-picker type="date" placeholder="选择日期" :format="item.props.format" v-model="item.value" :picker-options="endTime(item.gl)" style="width: 100%;"></el-date-picker>
               <div class="warn" v-if="item.isWarning"><warn-info :info="judgementWarnCpt(item)"></warn-info></div>
            </el-form-item>
...
function rulesFn(data) {
  ...
  export default {
    ...
    methods: {
      startTime(nextId){
         const value = this.ruleForm[nextId]? this.ruleForm[nextId]: ''
        return {
          disabledDate: time => {
          // 既不能大于当前日期 也不能小于结束日期
            if(value) {
      ...
      handleInput(val,item,id) {
       ...
       endTime(prevId) {
         const value =this.ruleForm[prevId]? this.ruleForm[prevId]: ''
        return {
        disabledDate: time => {
        // 既不能大于当前日期 也不能大于开始日期
           if(value) {
      ...
      judgementWarnCpt(item) {
        ...
        if(!String(item.value).length && typeof item.value !== 'number') {
          if(this.offsetTop === 0) {
