      ...
      <template v-slot:content>
        <el-form :hide-required-asterisk="true" label-width="100px" class="ruleForm my-form">
          <template v-for="(item, index) in list">
            <el-form-item :class="formItemCpt(item)" :ref="`${item.id}Ref`" :key="index">
              <label slot="label"
                >{{ item.title
              ...
              <el-select
                v-if="item.name === 'SelectInput' && !item.props.expanding"
                v-model="item.value"
                :placeholder="item.props.placeholder"
              >
                <el-option
              ...
              <el-checkbox-group
                v-if="item.name === 'MultipleSelect' && item.props.expanding"
                v-model="item.value"
              >
